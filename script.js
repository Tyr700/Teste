...
const API_BASE = "http://185.137.122.137:3001"; // ou sua URL do projeto
...
function displayCurrentUser() {
  if (currentUser) {
    document.getElementById("currentUserInfo").innerHTML = `
            ...
            <p><strong>Cadastro:</strong> ${new Date(
...
async function loadUsers() {
  try {
    ...
    if (response.usuarios && response.usuarios.length > 0) {
      usersList.innerHTML = response.usuarios
        .map(
          (user) => `
                <div class="user-item">
                    <strong>${user.nome}</strong><br>
                    <small>${user.email}</small><br>
                    <small>Cadastrado em: ${new Date(
                      user.dataCadastro
